...
export async function findWords(query) {
  // העדפת שפת התוצאות שנבחרה ב"הגדרות" (נשמרת ב-localStorage).
  let language = 'he'
  try {
    const saved = localStorage.getItem('ihaveaword_search_lang')
    if (saved === 'en' || saved === 'he') language = saved
  } catch { /* אין localStorage */ }

  // עברית = התנהגות רגילה. אנגלית = עוטפים את התיאור בהנחיה ברורה (באנגלית),
  // כך שה-AI יחזיר את המילים, ההגדרות והנימוקים באנגלית — גם אם התיאור בעברית.
  const payload =
    language === 'en'
      ? `Please find the most likely ENGLISH words for the following description. ` +
        `Write every "word", "definition" and "reason" in English, even if the description is written in Hebrew or another language.\n\n` +
        `Description: ${query}`
      : query
